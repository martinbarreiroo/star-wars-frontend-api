...
export default function StarWarsBrowser() {
  ...
  const fetchCharacters = async (page: number) => {
    try {
      ...
      const response = await axios.get<ApiResponse>(`${API_BASE_URL}/characters`, {
        params: {
          page: page,
          limit: 9,
        },
  ...
  const getGenderColor = (gender?: string) => {
    if (!gender) return "bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-300"
    ...
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        <div className="mb-8 text-center">
          ...
          {totalCharacters > 0 && (
            <p className="text-sm text-gray-400 mt-2">{totalCharacters} characters in the databank</p>
          )}
        </div>
                ...
                >
                  ...
                  <CardContent className="space-y-3">
                    ...
                    {character.description && (
                      <p className="text-xs text-gray-400 line-clamp-3">{character.description}</p>
                    )}
                  </CardContent>
